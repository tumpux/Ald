...
//import 'package:square_in_app_payments/models.dart' as mdl;

class AldreyCart extends StatefulWidget {
  final String data2;
  final String param2;
  ...
  AldreyCart({Key key, @required this.data2, @required this.param2}) : super(key: key);

  @override
  _AldreyCartState createState() => _AldreyCartState(data2, param2);
}

class _AldreyCartState extends State<AldreyCart> {

  String data2;
  String param2;

  _AldreyCartState(this.data2, this.param2);
  ...
  void initState() {
    ...
    //cartAmount = 0;
    //print ('Cart amount after get number of items = ${cartAmount}');
    buildInd = false;
  ...
  Widget _buildBody(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      ...
      stream: Firestore.instance.collection('carts')
          .where('acctFullName', isEqualTo:data2 )
   ...
   Future<bool> _havePendingOrder() async  {
    ...
    //return pendingOrder;
     ...
     if (pendingOrder == true) {
       ...
       RecordUserDetails record;
       await Firestore.instance.collection('userDetails')
           .where('acctFullName', isEqualTo:acctName )
       //.where('confirmedPurchase', isEqualTo: false)
           .getDocuments().then((data) {
         ...
         List<DocumentSnapshot> listDocSnapshot =data.documents ;
         listDocSnapshot.forEach((snap) {
            record = RecordUserDetails.fromSnapshot(snap);
         });
       });
        ...
        if (record == null)  {
         ...
         await Navigator.of(context).push(new CupertinoPageRoute(
             builder: (BuildContext context) => new Address(pAcctName : acctName, fromOrder: true,)));
       } else {
          showDialog(
            context: context,
            builder: (BuildContext context) {
